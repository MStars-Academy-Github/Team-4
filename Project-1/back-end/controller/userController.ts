...
const liked = async (req: Request, res: Response, next: NextFunction) => {
  ...
  const user = await Users.findOne({
    imgUrl: body.firstImg,
    firstName: body.firstUserName,
  });
  // liked user by first user
  const second = await Users.findOne({
    imgUrl: body.secondImg,
    firstName: body.secondUserName,
  });
  console.log(typeof body.firstImg, typeof body.firstUserName);
  if (user && second) {
    await Users.updateOne(
      { imgUrl: body.firstImg, firstName: body.firstUserName },
      { $push: { liked: body.secondId } },
    ...
    await Users.updateOne(
      { imgUrl: body.secondImg, firstName: body.secondUserName },
      {
        $push: {
